...
## האם חייבים "להכניס מפתחות לקוד"?

**לא חובה.** בפרויקט יש הגדרה מוטמעת (`lib/firebase.ts`) – האתר אמור לעבוד גם בלי `.env.local`.

**מומלץ לפרודקשן:** להעתיק את אותם ערכים לקובץ **`.env.local`** (העתק מ־`.env.local.example`) כדי לא לשמור עותק בקוד. אחרי שינוי `.env.local` חובה **להפסיק ולהריץ מחדש** `npm run dev`.
